    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="clearForm">
      <el-form ref="roleFormRef" :model="roleForm" :rules="rules">
        ...
        <el-form-item prop="menuIdList" label="权限设置" :label-width="formLabelWidth">
          <el-tree ref="menuRef" :data="menuList" style="width: 85%;" node-key="menuId" default-expand-all :props="menuProps" show-checkbox />
        </el-form-item>
      </el-form>
    ...
    saveRole() {
      ...
      this.$refs.roleFormRef.validate((valid) => {
        if (valid) {
          ...
          const halfCheckKeys = this.$refs.menuRef.getHalfCheckedKeys()
          this.roleForm.menuIdList = checkedKeys.concat(halfCheckKeys)
          // 提交保存请求
    ...
    openEditUI(id) {
      if (id == null) {
        ...
        roleApi.getRoleById(id).then(response => {
          ...
          console.log(response.data.menuIdList, 'response.data.menuIdList')
