...
阶段 1 ✅ → 阶段 2 ✅ → 阶段 3 ✅ → 阶段 4 ✅ → 阶段 5A ✅ → 阶段 5B ✅ → **阶段 6A ✅ 已完成** → **阶段 6B 兼容层清理已完成并落地** → **阶段 6C 旧后台入口隔离与存活清点已完成** → **阶段 6D 明确死代码清理已完成** → **阶段 6E 旧后台菜单表与遗留模块审计已完成** → **阶段 6F 断链修复与 test 僵尸菜单清理已完成** → **阶段 7A admin 控制台信息架构方案已定义** → **阶段 7B admin 菜单收敛与 P0 页面改造已完成** → **阶段 8 admin 冒烟测试补洞与边界加固已完成** → **阶段 9 举报与审核中心最小闭环已落地** → **阶段 10 前台举报入口与对象上下文联动已落地**。
...
- 举报与审核中心已从方案进入最小可用实现：
  - 新增 `sys_report` 主表，覆盖举报人、对象类型、对象 ID、对象标题、举报原因、补充说明、处理状态、处理人、处理备注、创建时间、处理时间
  - 对象类型当前支持：`作品`、`社区内容`、`反馈`、`订单`
  - 状态当前支持：`待处理`、`已处理`、`已驳回`
- 后端已新增：
  - `GET /sysReport/list`
  - `GET /sysReport/getById/{id}`
  - `PUT /sysReport/handle`
  - `POST /sysReport/add`
  - 权限策略保持最小改动：列表/详情/处理走 admin 校验；创建举报走登录校验，为后续前台接入留占位
- 前端已新增：
  - `src/api/report.js`
  - `src/views/report/index.vue`
  - admin 白名单与标题覆写已接入 `src/utils/adminConsole.js`
  - dashboard Hero 已增加“举报与审核”快捷入口
- 本地数据库已执行：
  - `artistsion-admin/sql/2026-04-15-phase9-report-center.sql`
  - 当前本地库已存在 `x_menu.menu_id=38/39` 与 `x_role_menu.id=564/565`
- 实际联调结果：
  - 普通用户 token 可成功创建举报
  - admin token 可查询、查看详情、处理举报
  - 浏览器侧已验证：admin 登录后能在侧边栏看到“举报与审核”，页面能打开详情抽屉和处理弹窗，处理结果会回写列表
- 下一轮可直接衔接：
  - 前台举报入口接线
  - 举报对象上下文联动（跳转到作品/帖子/订单/反馈详情）
  - 处理动作扩展（如封禁、下架、转工单）

### 举报前后台闭环（10）

- 前台举报入口已真正接线，不再停留在 admin 单边能力：
  - `views/work/detail.vue` 新增“举报作品”按钮
  - `views/posts/detail.vue` 新增“举报内容”按钮，并通过 `router/index.js` 新增 `/post/:id`
  - `views/center/orders.vue` 新增“举报订单”入口
  - `views/liuyan/liuyanyh.vue` 新增“举报反馈”入口
- 为避免各页各做一套表单，已抽出统一组件：
  - `src/components/ReportDialog/index.vue`
  - 统一承接 `targetType` / `targetId` / `targetTitle`
  - 统一原因选项、校验、提交成功反馈
  - 未登录时按现有站点策略跳 `/auth?redirect=当前页`
- 社区内容对象页已补齐：
  - `src/views/home/index.vue` 首页新增“社区精选”卡片区
  - 点击卡片进入 `/post/:id`
  - `posts/detail.vue` 内可直接发起举报
- admin 举报中心已增强对象上下文联动：
  - 列表页直接展示对象标题 + 对象 ID
  - 详情抽屉可查看对象类型、对象标题、举报原因、补充说明、举报人、创建时间、处理时间、处理备注、处理人
  - 新增“查看原对象”跳转
    - 作品 → `/work/:id`
    - 社区内容 → `/post/:id`
    - 反馈 → `/liuyan/liuyan?focusId=:id`
    - 订单 → `/order/orderadglqb?focusId=:id`
- 为支撑对象定位，这轮额外补了最小必要接口能力：
  - `SysOrderController.getList()` 支持 `id` 精确过滤
  - `api/order.js` 传递 `id`
  - `api/liuyan.js` 传递 `id`
  - `liuyan.vue` 与 `orderadglqb.vue` 支持读取 `focusId` 并以提示条展示“当前已根据举报上下文定位”
  - `MyWebConfig.java` 白名单新增 `/sysZuopin/getById/**`，保证帖子详情页可公开访问
- 本轮真实联调结果：
  - 使用本地普通用户测试账号成功创建 4 类举报：作品 `#10`、社区内容 `#9`、反馈 `#13`、订单 `#37`
  - 使用本地 admin 测试账号成功在 `/sysReport/list` 中查到上述 4 条记录
  - admin 成功处理其中 1 条订单举报，`/sysReport/getById/{id}` 返回状态已更新为“已处理”
  - 对象定位相关接口联调通过：
    - `GET /sysZuopin/getById/9` 匿名可访问
    - `GET /sysLiuyan/list?id=13&pageNo=1&pageSize=10` 返回 1 条
    - `GET /sysOrder/list?id=37&pageNo=1&pageSize=10&status=购物车1` 返回 1 条
- 下一轮可直接承接：
  - 把“查看原对象”从新开标签页升级为更明确的审核工作流
  - 在举报处理动作中增加“下架作品 / 转反馈工单 / 标记风险用户”等联动动作
  - 再考虑前台更多内容流的举报入口，而不是继续扩第二个 admin 模块
